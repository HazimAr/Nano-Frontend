import { Heading, HStack, Image, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Stack, Text, useDisclosure, useToast, VStack } from '@chakra-ui/react';
import Button from '@components/button';
import EmojiPicker from '@components/emojiPicker';
import Select from '@components/select';
import { useState } from 'react';

export function CreateReactionRoleModal({ reactionRole, setReactionRole, availableRoles, custom }): JSX.Element {
				...
				<ModalContent bg="bg.primary">
					...
					<ModalBody>
						<Stack spacing={5}>
							...
							<Stack spacing={5}>
								<HStack size="md">
									<Heading size="md">Users will recieve: </Heading>
									{role?.name && (
								...
								<HStack size="md">
									<Heading size="md">When reacted with: </Heading>
									{emoji?.img ? <Image src={emoji.img} w={10} /> : <Text fontSize="3xl">{emoji}</Text>}
								</HStack>
							</Stack>
						</Stack>
					</ModalBody>
							...
							<Button
								onClick={() => {
									if (!emoji && !emoji?.emoji_id) {
										toast({
											title: 'Error',
											description: 'Please Choose An Emoji.',
											status: 'error',
											duration: 3000,
											isClosable: true,
										});
